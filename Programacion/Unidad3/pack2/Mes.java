package net.diego.unit3.pack2;

import java.util.Scanner;

/* 
 * @autor Diego Jesus Muñoz Andrade
 * @company TecNM Campus la Piedad
 *
 * Problema:
 * Escriba un programa que lea un número entre 1 y 12, si el
 * número está en ese rango, despliega el nombre del mes
 * correspondiente y termina, en caso contrario, vuelve a pedir el
 * número.
 *
 * paquete 2
 * 
*/
public class Mes {
    public static void main(String[] args) {
        Scanner in = new Scanner(System.in);
        int mes;
        
        do {
            System.out.print("Ingresa un número entre 1 y 12: ");
            mes = in.nextInt();
        } while (mes > 12 && mes < 1);
        
        switch (mes) {
            case 1:
                System.out.println("Enero");
                break;
            case 2:
                System.out.println("Febrero");
                break;
            case 3:
                System.out.println("Marzo");
                break;
            case 4:
                System.out.println("Abril");
                break;
            case 5:
                System.out.println("Mayo");
                break;
            case 6:
                System.out.println("Junio");
                break;
            case 7:
                System.out.println("Julio");
                break;
            case 8:
                System.out.println("Agosto");
                break;
            case 9:
                System.out.println("Septiembre");
                break;
            case 10:
                System.out.println("Octubre");
                break;
            case 11:
                System.out.println("Noviembre");
                break;
            case 12:
                System.out.println("Diciembre");
                break;
            default:
                System.out.println("Mes no encontrado...");
                break;
        }
        
        in.close();
    }
}
